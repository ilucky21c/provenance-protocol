 ...
 * Not part of the standard; you must name the index you choose to trust.
 */
...
export interface GateOptions {
   ...
   * The agent must have signed the nonce with signAgentChallenge.
   */
...
export interface ProvenanceOptions {
  /** Base URL of the index you choose to trust. Required — there is no default. */
  apiUrl: string;
  cacheTTL?: number;
  onApiError?: 'throw' | 'allow' | 'deny';
}
...
export class Provenance {
   ...
   *
   * Protocol: send the agent a nonce, ask it to sign it with signAgentChallenge
   * (domain-separated form), then call this to verify the returned signature
   * against the public key the index holds.
   *
